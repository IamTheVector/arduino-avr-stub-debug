...
- If **GDB closes on its own** while debugging, check the **AVR-GDB** panel: the extension prints **exit code**, **signal**, and **stderr** when the shutdown was not intentional (e.g. serial drop, crash, COM stolen).
